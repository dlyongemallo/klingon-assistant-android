...
import org.tlhInganHol.android.klingonassistant.KlingonContentDatabase.Companion.bundledDatabaseVersion
...
class UpdateDatabaseService : JobService() {
    ...
    private inner class UpdateDatabaseTask : AsyncTask<Void?, Void?, Void?>() {
        protected override fun doInBackground(vararg params: Void): Void? {
            ...
            val sharedPrefs =
                PreferenceManager.getDefaultSharedPreferences(this@UpdateDatabaseService)
            ...
            try {
                BufferedReader(
                    InputStreamReader(
                        URL(Companion.MANIFEST_JSON_URL).openConnection().getInputStream()
                    )
                ).use { bufferedReader ->
                    val sb = StringBuffer()
                    var line: String?
                    ...
                    val updatedVersion =
                        sharedPrefs.getString(
                            KlingonContentDatabase.KEY_UPDATED_DATABASE_VERSION,  /* default */
                            installedVersion
                        )
                    ...
                    if (latest.compareTo(updatedVersion!!, ignoreCase = true) > 0) {
                        ...
                        val databaseZipUrl =
                            Companion.ONLINE_UPGRADE_PATH + latestObject.getString("path")
                ...
                this@UpdateDatabaseService.jobFinished(mParams, rescheduleJob)
            }
        ...
        private fun copyDBFromZipUrl(databaseZipUrl: String) {
            ...
            val inStream = if ("gzip" == urlConnection.contentEncoding) {
