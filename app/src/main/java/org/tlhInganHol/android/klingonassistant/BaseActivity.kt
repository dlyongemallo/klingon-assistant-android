...
open class BaseActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private val oneOffJobPermissionLauncher: ActivityResultLauncher<String> =
    ...
    private val scheduledJobPermissionLauncher: ActivityResultLauncher<String> =
        registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
            if (isGranted) {
    ...
    fun isHorizontalTablet(): Boolean {
        ...
        // }
        // return false
        return resources.getBoolean(R.bool.drawer_layout_locked)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // supportActionBar?.setIcon(R.drawable.ic_ka)

        // Display the title in Klingon font.
        val klingonAppName = SpannableString(
        ...
        val appNameView = headerView.findViewById<TextView>(R.id.app_name_view)
        val versionsView = headerView.findViewById<TextView>(R.id.versions_view)
        ...
        // // val drawerType = MenuDrawer.Type.BEHIND
        // if (isHorizontalTablet()) {
    ...
    override fun onResume() {
        ...
        if (sharedPrefs.getBoolean(
            ...
            if (updatedVersion?.compareTo(installedVersion ?: "", ignoreCase = true) ?: 0 > 0) {
                val hamburgerDot = findViewById<TextView>(R.id.hamburger_dot)
    ...
    private fun applyTypefaceToMenuItem(menuItem: MenuItem, enlarge: Boolean) {
        ...
        val spannableTitle: SpannableString = if (useKlingonUI && useKlingonFont) {
            ...
            if (menuItem.itemId == R.id.about) {
                ...
                // item.
                klingonTitle = klingonTitle.replace("-", "▶")
            }
            val spannable = SpannableString(klingonTitle)
            spannable.setSpan(
                KlingonTypefaceSpan("", klingonTypeface),
                0,
                spannable.length,
            ...
            if (menuItem.itemId == R.id.about) {
                ...
                val loc = spannable.toString().indexOf(appName)
                if (loc != -1) {
                    spannable.setSpan(
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        if (sharedPrefs.getBoolean(
            ...
            kwotdButton.isVisible = true

            val updateDatabaseButton = menu.findItem(R.id.action_update_db)
            updateDatabaseButton.isVisible = true

            val editLang = sharedPrefs.getString(
                Preferences.KEY_SHOW_SECONDARY_LANGUAGE_LIST_PREFERENCE,
                /* default */ Preferences.getSystemPreferredLanguage()
            )
            ...
            if (editLang != "NONE") {
                ...
                autotranslateButton.isVisible = true
            }
        }
    ...
    protected fun setDrawerContentView(layoutResId: Int) {
        val constraintLayout = findViewById<ConstraintLayout>(R.id.drawer_content)
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.pronunciation -> displayHelp(QUERY_FOR_PRONUNCIATION) // Show "Pronunciation" screen.
            R.id.prefixes -> displayHelp(QUERY_FOR_PREFIXES) // Show "Prefixes" screen.
            R.id.prefix_chart -> displayPrefixChart() // Show "Prefix chart" screen.
            R.id.noun_suffixes -> displayHelp(QUERY_FOR_NOUN_SUFFIXES) // Show "Noun Suffixes" screen.
            R.id.verb_suffixes -> displayHelp(QUERY_FOR_VERB_SUFFIXES) // Show "Verb Suffixes" screen.
            R.id.sources -> displaySources() // Show "Sources" screen.

            // Handle media.
            R.id.media_1 -> launchYouTubePlaylist(baseContext.resources.getString(R.string.media_1_list_id))
            R.id.media_2 -> launchYouTubePlaylist(baseContext.resources.getString(R.string.media_2_list_id))
            R.id.media_3 -> launchYouTubePlaylist(baseContext.resources.getString(R.string.media_3_list_id))
            R.id.media_4 -> launchYouTubePlaylist(baseContext.resources.getString(R.string.media_4_list_id))
            R.id.media_5 -> launchYouTubePlaylist(baseContext.resources.getString(R.string.media_5_list_id))
            R.id.media_6 -> launchYouTubePlaylist(baseContext.resources.getString(R.string.media_6_list_id))

            // Handle KLI activities here.
            R.id.kli_lessons -> launchExternal("http://www.kli.org/learn-klingon-online/")
            R.id.kli_questions -> launchExternal("http://www.kli.org/questions/categories/")
            ...
            */
            ...
            R.id.empire_union_day -> displaySearchResults(QUERY_FOR_EMPIRE_UNION_DAY)
            /*
             * R.id.idioms -> displaySearchResults(QUERY_FOR_IDIOMS)
             */
            R.id.curse_warfare -> displaySearchResults(QUERY_FOR_CURSE_WARFARE)
            R.id.nentay -> displaySearchResults(QUERY_FOR_NENTAY)
            /*
             * R.id.proverbs -> displaySearchResults(QUERY_FOR_PROVERBS)
             */
            R.id.military_celebration -> displaySearchResults(QUERY_FOR_QI_LOP)
            R.id.rejection -> displaySearchResults(QUERY_FOR_REJECTION)
            R.id.replacement_proverbs -> displaySearchResults(QUERY_FOR_REPLACEMENT_PROVERBS)
            R.id.secrecy_proverbs -> displaySearchResults(QUERY_FOR_SECRECY_PROVERBS)
            R.id.toasts -> displaySearchResults(QUERY_FOR_TOASTS)
            R.id.lyrics -> displaySearchResults(QUERY_FOR_LYRICS)
            R.id.beginners_conversation -> displaySearchResults(QUERY_FOR_BEGINNERS_CONVERSATION)
            R.id.jokes -> displaySearchResults(QUERY_FOR_JOKES)
    ...
    private fun launchYouTubePlaylist(listId: String) {
        ...
        intent.data = Uri.parse("http://www.youtube.com/playlist?list=$listId")
    ...
    private fun launchFacebook(groupId: String) {
        val intent: Intent = try {
            ...
            Intent(Intent.ACTION_VIEW, Uri.parse("fb://group/$groupId"))
        } catch (e: Exception) {
        ...
        if (!jobAlreadyExists) {
            ...
            if (isOneOffJob) {
                ...
                if (activeNetwork == null || !activeNetwork.isConnectedOrConnecting) {
                    ...
                    Toast.makeText(
                        this,
                        resources.getString(R.string.kwotd_fetching),
                        Toast.LENGTH_SHORT
                    ).show()
                }
                ...
                builder = JobInfo.Builder(
                    KWOTD_SERVICE_ONE_OFF_JOB_ID,
                    ComponentName(this, KwotdService::class.java)
                )
                ...
                // gives up.)
                builder = JobInfo.Builder(
                    KWOTD_SERVICE_PERSISTED_JOB_ID,
                    ComponentName(this, KwotdService::class.java)
                )
        ...
        if (!jobAlreadyExists) {
            ...
            if (isOneOffJob) {
                ...
                if (activeNetwork == null || !activeNetwork.isConnectedOrConnecting) {
                    ...
                    Toast.makeText(
                        this,
                        resources.getString(R.string.update_db_fetching),
                        Toast.LENGTH_SHORT
                    ).show()
                }
    ...
    companion object {
        ...
        const val QUERY_FOR_ABOUT = "boQwI':n"
        ...
        // private const val QUERY_FOR_IDIOMS = "*:sen:idiom"
        private const val QUERY_FOR_CURSE_WARFARE = "*:sen:mv"
        private const val QUERY_FOR_NENTAY = "*:sen:nt"
        // private const val QUERY_FOR_PROVERBS = "*:sen:prov"
