...
class EntryFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val entryTitle = rootView.findViewById<TextView>(R.id.entry_title)
        val entryBody = rootView.findViewById<TextView>(R.id.entry_body)
        ...
        if (!entry.isAlternativeSpelling() && showUnsupportedFeatures) {
            ...
            if (definition_DE.isNotEmpty() && definition_DE != otherLanguageDefinition) {
                expandedDefinition += "\nde: $definition_DE"
            }
            if (definition_FA.isNotEmpty() && definition_FA != otherLanguageDefinition) {
                // Wrap Persian text with RLI/PDI pair.
                expandedDefinition += "\nfa: \u2067$definition_FA\u2069"
            }
            if (definition_SV.isNotEmpty() && definition_SV != otherLanguageDefinition) {
                expandedDefinition += "\nsv: $definition_SV"
            }
            if (definition_RU.isNotEmpty() && definition_RU != otherLanguageDefinition) {
                expandedDefinition += "\nru: $definition_RU"
            }
            if (definition_ZH_HK.isNotEmpty() && definition_ZH_HK != otherLanguageDefinition) {
                expandedDefinition += "\nzh-HK: $definition_ZH_HK"
            }
            if (definition_PT.isNotEmpty() && definition_PT != otherLanguageDefinition) {
                expandedDefinition += "\npt: $definition_PT"
            }
            if (definition_FI.isNotEmpty() && definition_FI != otherLanguageDefinition) {
                expandedDefinition += "\nfi: $definition_FI"
            }
            if (definition_FR.isNotEmpty() && definition_FR != otherLanguageDefinition) {
                expandedDefinition += "\nfr: $definition_FR"
            }
        }
        ...
        if (showAnalysis) {
            ...
            if (components.isNotEmpty()) {
                ...
                analysisQuery += KlingonContentProvider.Entry.COMPONENTS_MARKER + components.replace(Regex("[{}]"), "")
            }
        ...
        val showTransitivityInformation = transitivity.isNotEmpty()
    ...
    protected fun processMixedText(ssb: SpannableStringBuilder, entry: KlingonContentProvider.Entry?) {
        val smallTextScale = 0.8f
        val useKlingonFont = Preferences.useKlingonFont(requireActivity().baseContext)
        val klingonTypeface = KlingonAssistant.getKlingonFontTypeface(requireActivity().baseContext)

        var mixedText = ssb.toString()
        var m = KlingonContentProvider.Entry.ENTRY_PATTERN.matcher(mixedText)
        while (m.find()) {
            ...
            val linkedEntry = KlingonContentProvider.Entry(query, requireActivity().baseContext)
    ...
    companion object {
        private const val INTERMEDIATE_FLAGS = Spanned.SPAN_EXCLUSIVE_EXCLUSIVE or Spanned.SPAN_INTERMEDIATE
